...
void computeVerticalSeams(int a[][HEIGHT]) { //WIDTH*HEIGHT 28*28 first calculation
	for (int i = 0; i < WIDTH; i++)   //initiate first row
		verticalSeamCal[i][0] = a[i][0];
	for (int y = 1; y < HEIGHT; y++) { //compare and find the smallest value, going up to h
		for (int x = 0; x < WIDTH; x++) {
			verticalSeamCal[x][y] = a[x][y];
			if (x == 0)
				verticalSeamCal[x][y] += min(verticalSeamCal[x][y-1], verticalSeamCal[x+1][y-1]);
			else if (x == WIDTH-1)
				verticalSeamCal[x][y] += min(verticalSeamCal[x][y-1], verticalSeamCal[x-1][y-1]);
			else
				verticalSeamCal[x][y] += min(verticalSeamCal[x-1][y-1], verticalSeamCal[x][y-1], verticalSeamCal[x+1][y-1]);

		}
	}
}

void computeHorizontalSeams(int a[][HEIGHT]) { //NEWWIDTH*HEIGHT 14*28 second calculation
	for (int j = 0; j < HEIGHT; j++)   //initiate first column
		horiontalSeamCal[0][j] = a[0][j];
	for (int x = 1; x < NEWWIDTH; x++) {
		for (int y = 0; y < HEIGHT; y++) {
			horiontalSeamCal[x][y] = a[x][y];
			if (y == 0)
				horiontalSeamCal[x][y] += min(horiontalSeamCal[x-1][y], horiontalSeamCal[x-1][y+1]);
			else if (y == HEIGHT-1)
				horiontalSeamCal[x][y] += min(horiontalSeamCal[x-1][y], horiontalSeamCal[x-1][y-1]);
			else
				horiontalSeamCal[x][y] += min(horiontalSeamCal[x-1][y-1], horiontalSeamCal[x-1][y], horiontalSeamCal[x-1][y+1]);
		}
	}
}

void removeVerticalSeams(int a[][HEIGHT]){ //W=WIDTH, get NEWWIDTH*HEIGHT 14*28 first remove
	computeVerticalSeams(a);
	//find best column
	for (int d = 0; d < WIDTH-NEWWIDTH; d++) {
		int bestCol = 0;
		for (int i = 0; i < NEWWIDTH; i++) {
			if (verticalSeamCal[bestCol][HEIGHT-1] > verticalSeamCal[i][HEIGHT-1])
				bestCol = i;
		}
		for (int y = HEIGHT-1; y >= 0; y--) {
			//transfer value to new matrix
			for (int x = 0; x < WIDTH; x++) {
				if (x == bestCol) {
					...
					newVerticalMatrix[x][y] = a[x][y];
			}
			// update best column for the next row
			if (y > 0) {
				float theMin = verticalSeamCal[bestCol][y-1];
				if (bestCol > 0 && verticalSeamCal[bestCol-1][y-1] <= theMin)
					bestCol = bestCol - 1;
				else if (bestCol < WIDTH-1 && verticalSeamCal[bestCol+1][y-1] <= theMin)
					bestCol = bestCol + 1;
			}
		}
	}
...
void removeHorizontalSeams(int a[][HEIGHT]){ //W=NEWWIDTH, get NEWWIDTH*NEWHEIGHT 14*14 second remove
	computeHorizontalSeams(a);
	//find best row
	for (int d = 0; d < HEIGHT-NEWHEIGHT; d++) {
		int bestRow = 0;
		for (int j = 0; j < NEWHEIGHT; j++) {
			if (horiontalSeamCal[NEWWIDTH-1][bestRow] > horiontalSeamCal[NEWWIDTH-1][j])
				bestRow = j;
		}
		for (int x = NEWWIDTH-1; x >= 0; x--) {
			//transfer value to new matrix
			for (int y = 0; y < NEWHEIGHT; y++) {
				if (y == bestRow)
					newMatrix[x][y]=a[x][y+1];   //when reach min best row shift to lower
				else
					newMatrix[x][y] = a[x][y];
			}
			// update best row for the next column
			if (x > 0) {
				float theMin = horiontalSeamCal[x-1][bestRow];
				if (bestRow > 0 && horiontalSeamCal[x-1][bestRow-1] <= theMin)
					bestRow = bestRow - 1;
				else if (bestRow < HEIGHT-1 && horiontalSeamCal[x-1][bestRow+1] <= theMin)
					bestRow = bestRow + 1;
			}
		}
	}
...
void seamCarving(int a[]) {
	for(int y=0; y<HEIGHT; y++){
		for (int x=0; x<WIDTH; x++){
			originMatrix[x][y] = a[x+(WIDTH*y)];
		}
	}
	removeVerticalSeams(originMatrix);//get 14*28 matrix
	removeHorizontalSeams(newVerticalMatrix);//get 14*14 matrix
	for(int y=0; y<NEWHEIGHT; y++){
			for (int x=0; x<NEWWIDTH; x++){
				newImage[x+(NEWWIDTH*y)] = newMatrix[x][y];
			}
		}
} */
